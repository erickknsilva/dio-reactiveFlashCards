...
import static dio.reactiveflashcards.domain.exception.BaseErrorMessage.GENERIC_METHOD_NOT_ALLOW;
import static org.springframework.http.HttpStatus.METHOD_NOT_ALLOWED;

@Slf4j
@Component
public class JsonProcessingExceptionHandler extends AbstractHandlerException<JsonProcessingException> {
    public JsonProcessingExceptionHandler(ObjectMapper objMapper) {
        super(objMapper);
    }


    @Override
    Mono<Void> handleException(ServerWebExchange exchange, JsonProcessingException ex) {
        return Mono.fromCallable(() -> {
                ...
                .doFirst(() ->
                        log.error("=== JsonProcessingException: Failed to map exception for the request {}",
